...
\begin{abstract}
  ...
  In this work, we study the relationship between online training signals and the optimal decay
...
All policies share the same gating and threshold mechanism.
To avoid premature decay, a \emph{gate step} of 7,000 (70\% of total steps)
is enforced: no policy can fire before this point.
At gate time, we collect metrics over a \emph{lookback window} spanning the
preceding 10\% of total steps (1,000 steps), which provides a stable
empirical distribution of each signal under near-converged stable-phase
dynamics.
Trigger thresholds are then set at the \textbf{3rd percentile} of this
distribution.
This low percentile was chosen empirically: it selects only the most
unusually quiet moments—where loss variance or gradient SNR has dropped to
a level rarely seen throughout the stable phase—without being so strict that
the condition never fires.
In practice this strikes a good balance between sensitivity (triggering
close to the true sweet spot) and robustness (ignoring transient dips).
With thresholds fixed, each policy monitors the live metric stream and fires
as soon as the condition is met:
\begin{itemize}
  \item \textbf{Loss-variance policy}: triggers when the current
        \texttt{loss\_variance} falls below the 3rd-percentile threshold
        derived from the lookback window.
  \item \textbf{Grad-SNR policy}: analogously triggers when \texttt{grad\_snr}
        falls below its 3rd-percentile threshold.
  \item \textbf{Combined policy}: requires \emph{both} conditions
        simultaneously, providing the most conservative trigger.
\end{itemize}
Decay length is fixed at 1,000 steps with final LR ratio 0.1.
We compare each policy against three fixed baselines that start decay at
70\%, 80\%, and 90\% of steps respectively—the last being the setting
recommended by~\citet{wen2024river}.
